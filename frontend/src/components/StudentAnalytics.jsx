  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Target size={20} />} title="Overall Progress" value={`${data.progress}%`} color="text-primary-400" bg="bg-primary-500/10" />
        <StatCard icon={<Clock size={20} />} title="Pending Assignments" value={data.pendingAssignments} color="text-amber-400" bg="bg-amber-500/10" />
        <StatCard icon={<CheckCircle size={20} />} title="Quizzes Passed" value={data.completedQuizzes} color="text-emerald-400" bg="bg-emerald-500/10" />
        <StatCard icon={<BookOpen size={20} />} title="Attendance Rate" value={`${data.attendancePercentage}%`} color="text-violet-400" bg="bg-violet-500/10" />
      </div>

      {/* Main Chart */}
      <div className="glass-panel p-6 rounded-2xl border border-dark-800">
        <h3 className="text-sm font-bold text-white mb-6 uppercase tracking-widest">Learning Activity</h3>
